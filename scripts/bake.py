...
Печёт ассеты TERMA.
  ...
  1. Кладёт кадры героя в выдачу КАК СНЯТЫ.
  2. Печёт конденсат для наведения и матовую наледь для ответов.

Градуировки здесь нет и быть не должно: кадр отдаётся как снят.

Масок по кадру героя здесь тоже больше нет. Область воды и передний
план вырезались под водяной приём и под перекрытие имени архитектурой —
и то и другое снято: в кадре героя лежит ОДНА фотография одним слоем.
Полигоны удалены вместе с потребителями, а не оставлены про запас.

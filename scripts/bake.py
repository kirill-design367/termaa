...
# ───────────────────────────────────────────────── локальная правка кадра ───
def lift_region(im: Image.Image, *, u0, u1, v0, v1, fu, fv, amount, protect) -> Image.Image:
    """
    Локальный подъём полутонов в области кадра.

    Нужен там, где чернила тёмные, а земля под ними тёмная: скрим ставить
    нельзя, значит правится сам кадр. Подъём идёт гаммой, а не
    множителем: множитель выбил бы света, гамма меньше единицы физически
    не может.

    Две вещи делают правку невидимой. Первая — растушёвка границ области
    (`fu`, `fv`): жёсткий край здесь читался бы прямоугольником поперёк
    фотографии. Вторая — защита самых тёмных пикселей (`protect`): в
    области стоят чёрные стойки павильона, и если поднять и их, они
    перестанут работать передним планом, а вместе с ними уйдёт глубина.
    """
    ...
    def ramp(n, lo, hi, f):
        ...
        m = np.clip((t - lo) / max(1e-6, f), 0, 1) * np.clip((hi - t) / max(1e-6, f), 0, 1)
        return m * m * (3 - 2 * m)

    mask = ramp(w, u0, u1, fu)[None, :] * ramp(h, v0, v1, fv)[:, None]

    lum = a @ np.array([0.2126, 0.7152, 0.0722], np.float32)
    # Ниже `protect` подъёма нет вовсе, выше — в полную силу.
    keep = np.clip((lum - protect) / 0.14, 0, 1)
    keep = keep * keep * (3 - 2 * keep)

    gam = 1.0 - amount * mask * keep
    return Image.fromarray(
        (np.clip(a, 0, 1) ** gam[:, :, None] * 255).astype(np.uint8), "RGB"
    )
...
def main():
    ...
    # Интерьер павильона поднят локально. Причина замерена, а не на глаз:
    # вордмарк идёт во всю ширину, и его правый конец приходится на
    # тёмное дерево интерьера — светлота 0.086, почти чёрные чернила
    # дают там 2.8:1 при требовании 4.5:1. Скрим поверх кадра запрещён,
    # значит правится сам файл. Направление подъёма, а не затемнения,
    # следует из выбора чернил: земля тёмная, чернила тёмные.
    gd = lift_region(gd, u0=0.795, u1=1.02, v0=0.16, v1=0.74,
                     fu=0.075, fv=0.10, amount=0.47, protect=0.15)
    ...
    # Передний план десктопного кадра: плита крыши и чёрные стойки
    # остекления. Кромки сняты по сетке — см. scripts/_roof.png в отчёте.
    build_fg_mask(desk.size, [
        # Плита крыши: наклонная нижняя кромка от дальнего угла вправо.
        [(0.522, 0.382), (0.622, 0.000), (1.000, 0.000), (1.000, 0.240),
         (0.775, 0.243), (0.700, 0.262), (0.640, 0.283), (0.585, 0.318),
         (0.545, 0.358)],
        # Стойки остекления. Ширины взяты из профиля светлоты, не на глаз.
        [(0.7640, 0.10), (0.7692, 0.10), (0.7692, 0.72), (0.7640, 0.72)],
        [(0.7846, 0.11), (0.7960, 0.11), (0.7960, 0.72), (0.7846, 0.72)],
        [(0.9330, 0.13), (0.9460, 0.13), (0.9460, 0.72), (0.9330, 0.72)],
    ], "fg-desktop")
    print("готово →", OUT)

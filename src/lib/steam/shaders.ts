...
void main() {
  ...
  float rare = s.b * pow(0.895, k);

  if (uActive > 0.5) {
    // Реакция локальная и с длинным градиентом: жёсткой границы у пятна
    // нет, на расстоянии оно гаснет полностью. Но она обязана читаться
    // с первого движения — прежние значения давали расхождение на
    // пределе различимости, и эффекта не было видно вовсе.
    //
    // Значения подобраны замером, а не на глаз: A/B-прогон с курсором и
    // без даёт среднюю разницу 2.04 уровня против 1.07 до правки. Дальше
    // усиливать бессмысленно — при вдвое большем смещении разница падает
    // до 1.30: выборка уезжает в некоррелированный шум, а разрежение
    // выедает пар там, где его и так мало.
    vec2 r = asp(uv - uP1);
    float d = length(r);
    float w = exp(-d * d / 0.0115);
...
vec3 warp(vec3 p, float dep, float h) {
  ...
  // Пропорции равные. Растяжения по одной оси нет: и вытягивание по
  // вертикали, и разное расхождение по x и y давали повторяющийся ритм —
  // пар читался как наложенная текстура, а не как среда.
  vec3 q = p * sc;
  q /= (1.0 + 0.42 * h);
  // Снос без периода: две несоизмеримые частоты не складываются в узор.
  q.x += (sin(p.y * 1.28 + dep * 5.1) + sin(p.y * 0.61 + dep * 2.3) * 0.6) * 0.16 * h;
...
float puffs(vec3 q, float h) {
  ...
  // Крупные разрежения: целые участки, где пара нет совсем. Берутся с
  // очень низкой частоты, поэтому режут не отдельные клубы, а области —
  // без них поток выглядит однородной пеленой.
  float gap = texture(uNoise, q * 0.19 + vec3(3.7, 1.9, 5.3)).r;
  v -= smoothstep(0.58, 0.30, gap) * 0.22;
...
/**
 * Вертикальный профиль потока.
 *
 * В покое — поток от источника внизу, сходящий на нет к верхней трети.
 * По прогрессу сцены он собирается в ПЛОТНЫЙ ФРОНТ и уходит вверх за
 * кромку кадра; за фронтом кадр очищается. Равномерного помутнения
 * экрана здесь быть не должно: движение обязано читаться направленным.
 */
float riseProfile(float h) {
  // Покой: источник внизу, верх чист.
  float rest = smoothstep(0.62, 0.02, h) * smoothstep(-0.10, 0.04, h);

  // Фронт: полоса плотности, идущая снизу вверх и выходящая за кадр.
  float frontY = mix(0.06, 1.55, uProgress);
  float front = smoothstep(frontY + 0.36, frontY - 0.04, h)   // выше фронта тает
              * smoothstep(frontY - 0.74, frontY - 0.28, h);  // ниже фронта чисто
  front *= smoothstep(-0.10, 0.04, h);

  // Переход из покоя во фронт — по началу прокрутки, а не мгновенно.
  return mix(rest, front * 1.45, smoothstep(0.08, 0.40, uProgress));
}

...
export function Scene() {
  useEffect(() => {
    ...
    const build = () => {
      ...
      // Фотография растворяется в паре до того, как липкая площадка
      // отлипнет. Иначе её нижняя кромка выезжает как ровная черта во
      // всю ширину — тот самый шов, только теперь в конце сцены.
      master.fromTo(
        '.hero__bg',
        { opacity: 1 },
        { opacity: 0, duration: hf * 0.26 },
        hf * 0.62,
      )

      // Объём не гасим: к концу прогресса фронт сам уходит за верхнюю
      // кромку и кадр очищается — гасить нечего, а гашение как раз и
      // читалось паузой. Снимаем только испечённый поток сцены, его
      // ниже подхватывает поток полотна.
      master.fromTo(
        '.hero__stage .flow',
        { autoAlpha: 1 },
        { autoAlpha: 0, duration: hf * 0.16 },
        hf * 0.8,
      )

      // Поток полотна густеет к концу сцены и заполняет кадр — это он и
      // есть переход. Ниже возвращается к базовой плотности.
      master
        .fromTo('.flow-page', { opacity: 0.45 }, { opacity: 0.95, duration: hf * 0.28 }, hf * 0.72)
        .to('.flow-page', { opacity: 0.45, duration: hf * 0.1 }, hf * 1.02)

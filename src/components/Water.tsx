 ...
 *
 * Отражение имени — часть приёма, а не отдельный слой: зеркальная
 ...
 * уходит от камеры, и отражение обязано укорачиваться. Прозрачность
 * 25 %: вода на кадре синего часа тёмная, и белому слову в ней есть где
 * читаться — на прежнем светлом кадре те же 25 % были бы грязью.
 *
 * Тёплые блики павильона лежат на воде полосами. Отдельного кода для
 * них нет и не нужно: они ЧАСТЬ ФОТОГРАФИИ, а приём смещает пиксель
 * фотографии — значит волна дробит и смещает сами блики. Ради этого
 * амплитуда у ближней кромки поднята: именно там блики и лежат.
 */
...
void main(){
  ...
  col = mix(col, rf.rgb, rf.a * 0.25 * uRefA * deep * m);
  ...
  useEffect(() => {
    ...
    const boot = () => {
      ...
      const layout = () => {
        ...
        /* Потолок по числу пикселей холста.
           На новом кадре бассейн занял низ во всю ширину, и площадка
           выросла втрое — с 0.34 до 1.01 Мпкс при 1920×1080. Сама по
           себе она не страшна, но на ретине множитель 2 даёт ещё
           вчетверо, и приём начинает считать 4 Мпкс на кадр ради воды,
           которая по определению размыта. Потолок срезает ровно этот
           случай и не трогает обычный экран: 1.01 < 1.2, множитель
           остаётся единицей. */
        const cap = 1.2e6
        const want = Math.min(2, window.devicePixelRatio || 1)
        const px = rect.w * rect.h * want * want
        const dpr = px > cap ? Math.max(1, want * Math.sqrt(cap / px)) : want
